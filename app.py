...
st.markdown("""
    <style>
    html, body, [class*="css"] {
        font-size: 20px !important;  /* 基本の文字サイズを大きめに */
    }

    label, .stRadio > div, .stSelectbox > div, .stNumberInput > div {
...
def show_barcode():
    ...
    st.write(f"次に、このバーコード（ID: {unique_id}）をカメラに読み取らせてください：")
...
def show_survey():
    ...
    # データベース保存 or JSON化して送るなどもこの形なら簡単です
        if not st.session_state.submitted:
            if st.form_submit_button("送信して撮影へ進む"):
